...
const EasyCrop = forwardRef((props, ref) => {
  ...
  //after complete crop, set crop Pixel to croppedAreaPixels
...
const ImgCrop = forwardRef((props, ref) => {
  ...
  //prepare pass user defined call back to cropper
  const cb = useRef({});
  ...
  //generate uploadComponent only when children changed
    ...
    return {
      ...
      props: {
        ...
        beforeUpload: (file, fileList) => {
          //if user pass in before crop props and it's false, then not open modal
  ...
  //when close modal, reset to init
  const onClose = () => {
    setImage('');
  ...
  //when cancel modal, call user defined proops and reset
  ...
  return getComponent(modalTitle||'EDIT IMGAGE')
});
